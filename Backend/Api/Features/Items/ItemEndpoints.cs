...
public static class ItemEndpoints
{
    ...
    public static void MapItemsEndpoints(this WebApplication app)
    {
        ...
        // Get by ID
        group.MapGet("/{id}", async Task<Results<Ok<ItemDetailsDto>, NotFound>> (Guid id, ApiContext dbContext) =>
        {
        ...
        // Post
        group.MapPost("/", async (CreateItemDto itemData, ApiContext dbContext) =>
        {
            Item item = new()
            {
                Name = itemData.Name,
        ...
        // Put
        group.MapPut("/{id}", async Task<Results<NoContent, NotFound>> (Guid id, UpdateItemDto newData, ApiContext dbContext) =>
        {
        ...
        // Delete
        group.MapDelete("/{id}", async (Guid id, ApiContext dbContext) =>
        {
