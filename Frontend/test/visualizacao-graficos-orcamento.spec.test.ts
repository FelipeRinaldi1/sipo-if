...
function buildAreaSeries(evolucaoMensal: EvolucaoMensalItem[]): SeriesItem[] {
  return [
...
function buildBarSeries(evolucaoMensal: EvolucaoMensalItem[]): SeriesItem[] {
  return buildAreaSeries(evolucaoMensal);
}

function buildCompSeries(acoes: AcaoItem[]): SeriesItem[] {
  return [
...
test('AC-014: Dados de elemento de despesa têm campos necessários para o donut @spec:AC-014', () => {
  const elementosDespesa: ElementoDespesaItem[] = [
  ...
  const series: number[] = elementosDespesa.map(e => e.totalPago);
  const labels: string[] = elementosDespesa.map(e => e.elemento);
  ...
  const totalPct: number = elementosDespesa.reduce((s, e) => s + e.porcentagemDoTotal, 0);
...
test('AC-015: Top 5 fornecedores estão ordenados de forma decrescente @spec:AC-015', () => {
  const topFornecedores: FornecedorItem[] = [
  ...
  const barSeries: number[] = topFornecedores.map(f => +(f.totalPago / 1000).toFixed(2));
